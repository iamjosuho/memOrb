...
| # | 問題 | 修法 |
| :-- | :--- | :--- |
| A | `scripts/reset-sandbox.sh` 只複製 `fixtures/memorbs/*` 到 `sandbox/`，`memorbs/Long-Term/` 遷移後從沒同步更新，導致 `npm run reset-sandbox` 建出來的沙盒**沒有任何 Long-Term 樣本資料**（Alex.md、Orgs、Projects 全部消失），本地測試 memorb-query/lint/ingest 會拿到不完整的假資料。 | 補上 `cp -r fixtures/memorbs/Long-Term/* sandbox/memorbs/Long-Term/`，已測試確認 `sandbox/memorbs/Long-Term/{Orgs,People,Projects}` 都正確生成。 |
| B | 上一輪把 `recording-transcription` 的 `memorbs/people/*/*.md`（兩層萬用字元）改成 `memorbs/Long-Term/People/*.md`（我自己少改掉一層），單層 glob 抓不到 `business-card-ingestion` 建的巢狀 Page Bundle（`memorbs/Long-Term/People/{公司}/{姓名}/{姓名}.md`），比修之前還更抓不到人物檔案。 | 改成 `find "$VAULT/memorbs/Long-Term/People" -name "*.md"`（遞迴），並註明「不能只掃一層」。 |

## 2. P1 — 架構決策/內容需要對齊（建議優先處理）

| # | 狀態 | 問題 | 為什麼重要 | 涉及檔案 |
| :-- | :-- | :--- | :--- | :--- |
| 1 | ✅ **已解決（2026-07-28）** | **People 頁面「扁平 vs 公司巢狀」沒有統一**：`business-card-ingestion` 建立 `memorbs/Long-Term/People/{公司}/{姓名}/{姓名}.md`（公司資料夾巢狀），`memorb-domain-query` 的 Dataview 查詢也假設同一種巢狀（`contains(file.folder, "memorbs/Long-Term/People/Acme")`）；但 `memorb-conventions` SSOT 明文說 Long-Term 實體頁「預設為單一頁面」，`recording-transcription`／`m365-meeting-note`／`memorb-ingest` 也都假設扁平 `memorbs/Long-Term/People/{姓名}.md`。同一個人透過不同管道建檔，可能落在兩個不同路徑，造成重複頁、`memorb-lint` 抓不到孤兒、`memorb-query` 讀不到既有記錄。 | `memorbs/Long-Term/People/` 已統一規範為嚴格扁平 `PascalName.md`（或 bundle），解決巢狀不一致問題。 | `memorb-conventions`、`business-card-ingestion`、`memorb-domain-query`、`recording-transcription`、`memorb-ingest` |
| 2 | ✅ **已修復**（2026-07-26） | `memorb-query` 的「Three-Layer Responsibility」與 Red Flags 原本寫「`memorbs/`: How this changed our understanding of people/projects/topics」——memorbs/Long-Term/ 遷移前的舊說法。 | `memorb-query` 幾乎每次對話都會用到，是觸發頻率最高的 core skill 之一。已改為區分「`memorbs/Long-Term/`（實體）／`memorbs/HQ/`（原則/術語）」，並在 `scripts/lint-skills.js` 的 `DEPRECATED_PATTERNS` 新增一條規則，往後若再寫回這種舊措辭會被 lint 自動抓出來（已用舊句子測過會觸發）。 | `skills/core/memorb-query/SKILL.md`、`scripts/lint-skills.js` |
| 3 | ✅ **已解決（不再適用）** | 原本是「SSOT 沒收錄 `memorbs/meeting-note/`，但三個 skill 都在寫」。2026-07-26 廢除該命名空間——會議記錄不是獨立分類，而是一顆 orb，逐字稿放進同一個 bundle 資料夾。三個 skill 已改指 OrbTrack。 | 無需再處理，並已加 `DEPRECATED_PATTERNS` 防止寫回。 | — |
| 4 | ✅ **已解決（2026-07-28）** | 同理，`memorbs/glossary.md`（`memorb-ingest`、`recording-transcription` 都會讀寫）也沒被 SSOT 收錄。 | `memorbs/HQ/glossary.md` 已補進 SSOT 與 fixtures。 | `skills/core/memorb-conventions/SKILL.md` |
...
| 6 | ✅ **已解決（2026-07-28）** | `memorb-conventions` 的「範本路徑」表沒有 People／Org 範本這一列，但 `business-card-ingestion` 直接引用了 `Templates/People Template.md`——這個檔名沒有出現在唯一標準文件裡。文件本身已有「待固化」TODO，但沒具體點名缺哪幾列。 | `memorbs/Templates/`（收錄 5 個 master templates：People, Org, Project, Core, Belief）已補進 SSOT 與範本庫。 | `skills/core/memorb-conventions/SKILL.md` |
...
| 12 | ⏳ 待處理 | `skills/memorb/SKILL.md` 路由表對 `memorb-conventions` 的一句話描述只提到 `memorbs/`，沒提到 vault 根目錄的 `memorbs/Long-Term/`／`memorbs/Islands/` 也歸它管。 | 補一下這句話，涵蓋 memorbs/Long-Term/Islands。 |
| 13 | ⏳ 待處理 | `scripts/reset-sandbox.sh` 目前是手動列出要複製哪些頂層 fixture 資料夾（這次才漏掉 `memorbs/Long-Term/`，見上面 A 項）。 | 可以考慮改成 `cp -r fixtures/*/  sandbox/` 這種不用每次手動加新頂層資料夾的寫法。 |

## 5. 沒發現問題、可以放心的部分

- `plugin.json` 註冊：全對應，無孤兒無缺漏。
- Frontmatter：全部 SKILL.md 只有 `name`/`description` 兩欄，符合 `writing-memorb-skills` 規範，長度也都通過 lint 的最短長度檢查。
- `memorb-lint`／`memorb-forgetter`／`island-reclamation`／`orbtrack-triage`（路由已補）：路徑與架構都已對齊 `memorbs/Long-Term/` SSOT，內容邏輯沒發現矛盾。
...
| Skill | 問題 |
| :--- | :--- |
| `memorb-born` | Q1-Q4 訪談流程寫得籠統（「using `ask_question` tool if running interactively」是個沒展開的條件句，非互動時該怎麼辦沒說）。**「沒建使用者原生資料夾」那半條已不再是問題**——2026-07-26 定案 core 只擁有 `memorbs/`／`memorbs/Islands/`／`memorbs/Long-Term/`，不建也不依賴原生資料夾，born 現在的行為才是對的。仍待處理：從沒被其他 skill 使用過的 `AGENT.md`。 |
| `obsidian-cli` | 讀起來像是直接搬 Obsidian CLI 官方文件，完全沒有跟這個 vault 的架構（`memorb-conventions`、`memorbs/Long-Term/`、frontmatter schema）掛勾，也沒說明「什麼時候該用 `obsidian` CLI、什麼時候該直接用 Read/Edit 工具」。觸發描述也沒有走其他 skill 慣用的「觸發詞：」格式。概念上比較像一份工具速查表，不是一個整合進 memOrb 工作流的 skill。 |

### 🟡 堪用但可加強

| Skill | 問題 |
| :--- | :--- |
| `memorb-conventions` | SSOT 本身品質最高，但文件內有 2 處自承的「待固化」：Project 範本要補 Authority Control 欄位、`memorbs/Long-Term/`／`memorbs/HQ/` 頁面沒有固定範本檔。 |
...
| 廢除 `memorbs/meeting-note/` | 會議記錄不是獨立分類，是一顆 orb，逐字稿放同一個 bundle 資料夾，落 OrbTrack 等 triage |
| 確立權責邊界 | core 只擁有 `memorbs/`／`memorbs/Islands/`／`memorbs/Long-Term/`，必須能在只有這三個資料夾的 vault 上跑完；原生資料夾存在才寫、不建、不依賴 |
| 原始素材 | 進 orb 自己的 bundle 資料夾，跟 orb 同生共死；廢除 `Resources/`、`OrbTrack/Attachments/` 與 `source:` 欄位三套並存機制。只有文件檔進 vault，音訊影片留在外部 |
| 單一根目錄 | `Islands/`、`Long-Term/` 移入 `memorbs/`。memOrb 只讀不寫使用者資料夾，只往 `memorbs/` 裡面寫，extension 亦同 |
| 實體 vs 事件 | 實體頁單數不帶日期；事件 orb 帶日期且不合併，重複發生就是兩顆，都連回實體頁 |

尚未處理，另開討論：`memorbs/Long-Term/People/` 巢狀 vs 扁平路徑不一致（P1 #1）、根目錄 `Dump/` 與 `memorbs/Dump/` 撞名、SKILL.md 語言統一。
