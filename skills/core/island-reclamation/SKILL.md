...
（原本的「B 巢狀分類法」——Island 下再開 Projects/Resources/Archives 子資料夾——已停用：Project 一律住在 `Long-Term/Projects/`，跟任何 Island 都是連結關係，不物理巢狀。）
...
1. `memorbs/MEMORY.md` 索引加一行
2. `memorbs/log.md` 開頭 append 一筆 `ingest | 建立 Island「{名稱}」`，含決策摘要與影響頁面
...
| 「memory 同步下次再做」 | 命名與結構決策理由要當場寫進 MOC，不是事後才補；`MEMORY.md`／`log.md` 索引也不能拖到下次，拖了就對不上實際建立時間。 |
